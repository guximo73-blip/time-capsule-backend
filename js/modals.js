...
function openForm(itemData) {
    ...
    if (itemData) {
        ...
        const statusRadios = document.querySelectorAll('input[name="formStatus"]');
        statusRadios.forEach(r => r.checked = false);
        if (itemData.thumbnail || itemData.videoEmbed) {
            document.querySelector('input[name="formStatus"][value="已补完"]').checked = true;
        } else {
            document.querySelector('input[name="formStatus"][value="待补"]').checked = true;
        }
    } else {
        editModeId = null;
...
scheduleForm.onsubmit = async function(e) {
    ...
    let links = '[]';
    if (linksRaw) {
        if (linksRaw.startsWith('http')) {
            links = JSON.stringify([{ icon: '🔗', text: '查看', link: linksRaw }]);
        } else {
            try { const parsed = JSON.parse(linksRaw); if (!Array.isArray(parsed)) throw new Error();
                links = JSON.stringify(parsed); } catch { alert('链接格式错误，请输入URL或JSON数组'); return; }
        }
    }
